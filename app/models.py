...
class TwitterAPIWrapper(object):
    ...
    def __init__(self):
        ...
        this.api = twitter.Api(consumer_key=keys['consumer_key'], consumer_secret=keys['consumer_secret'], access_token_key=keys['access_token_key'],  access_token_secret=keys['access_token_secret'], sleep_on_rate_limit=True) # NOTE: setting sleep_on_rate_limit to True here means the application will sleep when we hit the API rate limit. It will sleep until we can safely make another API call. Making this False will make the API throw a hard error when the rate limit is hit.
    ...
    def search(self, search_term, since=None, until=None, geocode=None):
        tweets = this.api.GetSearch(term=search_term, since=since, until=until, geocode=geocode)
        return tweets
