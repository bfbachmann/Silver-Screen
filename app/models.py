...
class TwitterAPI(object):
    ...
    def search_movie(self, movie):
        ...
        """
        if not movie.Title or (not isinstance(movie, Movie)):
            return None
        ...
        print('Searching Twitter for \"' + movie.SimplifiedTitle + '\"')

        for diff in range(0, 6):
            futures.append(executor.submit(self._make_request, movie.SimplifiedTitle, current_datetime, diff, imdbID))
...
class Movie(models.Model):
    ...
    def fillWithJsonObject(self, jsonObject):
        ...
        if jsonObject is not None:
            ...
            if self.Title:
                tc = TitleCleaner()
                self.SimplifiedTitle = tc.clean_title(self);
...
class TitleCleaner(object):
    ...
    def clean_title(self, movie):
        new_title = movie.Title
        simplified = False

        if self.__is_common(movie.Title):
            new_title = 'movie ' + new_title

        beforeChange = new_title
        ...
        if ':' in beforeChange:
            splitStr = beforeChange.rpartition(':')[0]
            if len(splitStr) > len(new_title) or not simplified:
                new_title = splitStr
                simplified = True
